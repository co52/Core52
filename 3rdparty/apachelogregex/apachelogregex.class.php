 ...
 *
 * Changes by Jonathon Hill for Core52:
 * =========================================
 * Version 1.2.1a - 12/16/2010
 *   + Fixed a bug when log fields are separated by a ':' instead
 *     of a space
 */
...
class ApacheLogRegex {
	...
	private function _parse_format() {
		...
		foreach(preg_split('/([: ])/', $this->_format, -1, PREG_SPLIT_DELIM_CAPTURE) as $i => $element)
		{
			if($i % 2 == 1) {
				$regex_elements[] = $element;
				continue;
			}
