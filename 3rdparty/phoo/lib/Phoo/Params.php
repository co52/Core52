...
class Params
{
    ...
    protected $_paramsUnescaped = array();
    ...
    /**
     * Set/get unescapted parameters
     *
     * @param array $params
     * @return Params
     */
    public function unescaped(array $params = array())
    {
    	if(0 == count($params)) {
            return $this->_paramsUnescaped;
        }
        $this->_paramsUnescaped = array_unique($this->_paramsUnescaped + array_flip($params));
        return $this;
    }
    ...
    public function signature()
    {
        ...
        // Note: All parameters must be URI-escaped before being added to the query.
        // Parameters should be URI-escaped in the query, but NOT when generating the signature.
        ksort($this->_params);
     ...
     */
    public function queryString($includeSignature = TRUE, $includePcode = TRUE, $separator = '&')
    {
        $this->checkRequiredParams();
        $params = $this->_params;
        ksort($params);
        if($includePcode) {
        	$params = array('pcode' => $this->_partnerCode, ) + $params;
        }
        
        if(empty($this->_paramsUnescaped)) {
        	
        	// no unescaped parameters, so use the old method (faster)
	        ...
	        foreach($params as $k => $v) {
	        	$str[] = (in_array($k, $this->_paramsUnescaped))?
	        		"$k=$v" :
	        		sprintf('%s=%s', $k, rawurlencode($v));
	        }
	        $queryString = implode($separator, $str);
	        
        }
        
        if($includeSignature) {
        	$queryString .= $separator."signature=".$this->signature();
        }
