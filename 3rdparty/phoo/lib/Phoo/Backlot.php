...
class Backlot extends APIWrapper
{
    ...
    protected $_apiEndpoints = array(
        'query' => 'http://api.ooyala.com/partner/query',
    ...
    public function setAttributes($params, array $attrs = array())
    {
        ...
        $response = $this->client()->get($this->_apiEndpoints['attributes'], $params->queryString());
        if($response->body() !== 'ok') {
        	throw new Exception\Http\BadRequest($response->body());
        }
    }
     ...
     * @return \Phoo\Response
     */
    public function assignPlayer($params)
    {
        $params = $this->toParams($params);
        $params->mode = 'assign';
