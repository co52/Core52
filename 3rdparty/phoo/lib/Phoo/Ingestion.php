...
class Ingestion extends APIWrapper
{
    ...
    protected $_apiEndpoints = array(
		...
		'liveStream' => 'http://api.ooyala.com/ingestion/create_live_stream',
    );
	...
	{		
		$params = $this->toParams($params)
            ->required(array('pcode', 'expires', 'primary_encoder_ip', 'password', 'width0', 'height0', 'bitrate0', 'signature'));

		return $this->client()->post($this->_apiEndpoints['liveStream'] . '?' . $params->queryString());
