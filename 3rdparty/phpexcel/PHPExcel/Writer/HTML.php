...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
				if ($drawing->getCoordinates() == $coordinates) {
					...
					$html .= PHP_EOL;
					//$html .= '		<img style="position: absolute; left: ' . $drawing->getOffsetX() . 'px; top: ' . $drawing->getOffsetY() . 'px; width: ' . ($drawing->getWidth()*0.7) . 'pt; height: ' . ($drawing->getHeight()*0.7) . 'pt;" src="' . $filename . '" border="0" width="' . ($drawing->getWidth()*0.7) . '" height="' . ($drawing->getHeight()*0.7) . '" />' . PHP_EOL;
					$html .= '		<img style="'.
					'position: relative; left: ' . $drawing->getOffsetX() . 'px; top: ' . $drawing->getOffsetY() . 'px; '.
					'width: ' . ($drawing->getWidth()*0.7) . 'pt; height: ' . ($drawing->getHeight()*0.7) . 'pt;" '.
					'src="' . $filename . '" border="0" />' . PHP_EOL;
				}
			}
		}
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		// Disabling this line fixes a blank page issue
		// (see http://phpexcel.codeplex.com/discussions/242617#PostDetailsCell_667968)
		//$css['table']['page-break-after'] = 'always';

		// .gridlines td { }
		$css['.gridlines td']['border'] = 'none';
		...
		foreach ($sheets as $sheet) {
			...
			while($column++ < $highestColumnIndex) {
				...
				$css['table.sheet' . $sheetIndex . ' td.col' . $column]['width'] = '42pt';
			}

			// col elements, loop through columnDimensions and set width
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					$width = PHPExcel_Shared_Drawing::pixelsToPoints($width) / 5;
					$column = PHPExcel_Cell::columnIndexFromString($columnDimension->getColumnIndex()) - 1;
					$this->_columnWidths[$sheetIndex][$column] = $width;
					//$css['table.sheet' . $sheetIndex . ' td.col' . $column]['width'] = $width . 'pt';
					//$css['table.sheet' . $sheetIndex . ' td.col' . $column]['width'] = $width . 'px';
					$css['table.sheet' . $sheetIndex . ' td.col' . $column]['width'] = $width . '%';

					if ($columnDimension->getVisible() === false) {
						$css['table.sheet' . $sheetIndex . ' td.col' . $column]['visibility'] = 'collapse';
						$css['table.sheet' . $sheetIndex . ' td.col' . $column]['*display'] = 'none'; // target IE6+7
					}
				}
			}
	...
	private function _generateTableHeader($pSheet) {
		...
		while($i++ < $highestColumnIndex) {
			if (!$this->_useInlineCss) {
				$html .= '		<td class="col' . $i . '">&nbsp;</td>' . PHP_EOL;
			} else {
				$style = isset($this->_cssStyles['table.sheet' . $sheetIndex . ' td.col' . $i]) ?
					$this->_assembleCSS($this->_cssStyles['table.sheet' . $sheetIndex . ' td.col' . $i]) : '';
				$html .= '		<td style="' . $style . '">&nbsp;</td>' . PHP_EOL;
			}
		}
